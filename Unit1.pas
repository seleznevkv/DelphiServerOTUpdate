...
dm.OTSet.Append;
   ...
   filestatus.Caption:='Необходимо выбрать файл!';
...
if Application.MessageBox('Вы уверены, что хотите удалить запись?','Удаление',mb_yesno)=idyes then
begin
...
showmessage('Не удается подключиться к базе данных!');
end;
try
dm.OTSet.Close;
...
showmessage('Не удается получить данные!');
...
showmessage('Возникла ошибка при обновлении!');
...
begin
 if application.MessageBox('Такой файл уже существует! Вы точно хотите его перезаписать?','Файл существует',mb_yesno)=idyes then
  begin
     try
...
CopyFile(pchar(fname),pchar('\\192.168.76.100\Server\www\other\InstructionsDOCs\'+PageEdit.Text+'\'+ExtractFileName(fname)),false);
 ...
 fileservexist;
   ...
   ShowMessage('Произошла ошибка при копировании файла или файл больше не существует на сервере');
 end;
   end;
      end
      else showmessage('Выберите файл и заполните текстовые поля!');
...
procedure TForm1.fileservexist();
 ...
 FileStatus.Caption:='Файл на сервере!';
 end
 else  begin
 FileStatus.Font.Color:=clred;
 FileStatus.Caption:='Файл не найден!'
 end;
